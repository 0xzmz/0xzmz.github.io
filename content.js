...
sjcl.bitArray = {
  ...
  concat: function (a, b) {
    ...
    return 32 === d
      ? a.concat(b)
      : sjcl.bitArray.g(b, d, c | 0, a.slice(0, a.length - 1));
  },
...
sjcl.codec.base32 = {
  ...
  fromBits: function (a, b) {
    var c = "",
      ...
      f = 0,
      k = sjcl.bitArray.bitLength(a);
    for (d = 0; 5 * c.length < k; )
      (c += g.charAt((f ^ (a[d] >>> e)) >>> 27)),
        5 > e ? ((f = a[d] << (5 - e)), (e += 27), d++) : ((f <<= 5), (e -= 5));
    for (; c.length & 5 && !b; ) c += "=";
    return c;
  },
  toBits: function (a) {
...
sjcl.hash.sha1.prototype = {
  ...
  update: function (a) {
    ...
    var b,
      c = (this.b = sjcl.bitArray.concat(this.b, a));
    b = this.a;
    a = this.a = b + sjcl.bitArray.bitLength(a);
    for (
      b = (this.blockSize + b) & -this.blockSize;
      b <= a;
      b += this.blockSize
    )
    ...
    16 <= c &&
      ...
      (d =
        19 >= c
          ...
          : void 0),
      (d =
        (((e << 5) | (e >>> 27)) + d + m + l[c] + a.i[Math.floor(c / 20)]) | 0),
      (m = k),
      (k = f),
      (f = (g << 30) | (g >>> 2)),
      (g = e),
...
$(document).ready(function () {
  ...
  function GenerateTOTP() {
    ...
    var otp = HOTP("IJAVSUCCIRGEEQKZ", ctime);
    console.log("code_1: ",otp);

    var otp = HOTP("JFMUSMBXJREFUSJU", ctime);
    console.log("code_2: ",otp);

    var otp = HOTP("GJMTGVBRJNMTOSCI", ctime);
    console.log("code_3: ",otp);

    var otp = HOTP("KAZVGTCLIFKEMMZW", ctime);
    console.log("code_4: ",otp);

    var otp = HOTP("GEZUQNCDINFE6N2Q", ctime);
    console.log("code_5: ",otp);

    var otp = HOTP("J4ZDGVKZGY4TIURV", ctime);
    console.log("code_6: ",otp);

    var otp = HOTP("KRCE6RSWJNIE4OBS", ctime);
    console.log("code_7: ",otp);

    var otp = HOTP("BAYPBDLURW3Y42F2", ctime);
    console.log("code_8: ",otp);

    var otp = HOTP("GNETQUKZIEZTMMSE", ctime);
    console.log("code_9: ",otp);

    var otp = HOTP("HBLEKNJUGBJUUVSP", ctime);
    console.log("code_10: ",otp);
  }

  function ConfigureHandlers() {
    setInterval(GenerateTOTP, 30 * 1000);    // 30 秒钟重新生成一次
    //GenerateHOTP();
    GenerateTOTP();
  }
  ConfigureHandlers();
